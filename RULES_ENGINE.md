...
- ≤1 active (non-eliminated) match player remains, or
- **Pool** (or the **final** deal of a fixed-length POINTS/DEALS match): the deal ends with ≤1 `PLAYING` seat after one or more `DROP`s (last-player-standing walkover → `MATCH_ENDED`), or
- **POINTS / DEALS**: `dealNumber >= dealsPerMatch` (lowest cumulative score among still-active seats wins).

**DEALS / POINTS with deals remaining:** a heads-up `DROP` ends only the *deal* (dropper gets drop points; opponent wins the deal). The match enters `BETWEEN_DEALS` and continues via `START_NEXT_DEAL` (or the auto countdown). Do **not** treat that screen as match-over — the client’s **Play Again** button is only for `MATCH_ENDED` and returns to the lobby.
...
| `DROP` | — | Your turn, `AWAITING_DRAW` (before drawing). Ends the deal when ≤1 `PLAYING` seat remains. **Pool** (or final fixed deal): match walkover → `MATCH_ENDED`. **DEALS/POINTS with deals left:** `DEAL_RESULT` / `BETWEEN_DEALS` → continue with `START_NEXT_DEAL`. |
