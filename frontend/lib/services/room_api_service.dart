...
/// REST client for `/api/v1/rooms/*` — 401 auto-refresh via [ApiClient].
class RoomApiService {
  ...
  Future<CreatedRoom> joinRoom({String? jwt, required String roomCode}) async {
    final response = await _client.post(
      ApiConfig.roomJoinUri(roomCode),
      accessTokenOverride: jwt,
    );
  ...
  Future<CreatedRoom> getRoom({String? jwt, required String roomCode}) async {
    final response = await _client.get(
      ApiConfig.roomUri(roomCode),
      accessTokenOverride: jwt,
    );
  ...
  Future<void> leaveRoom({String? jwt, required String roomCode}) async {
    final response = await _client.post(
      ApiConfig.roomLeaveUri(roomCode),
      accessTokenOverride: jwt,
    );
  ...
  Future<void> cancelRoom({String? jwt, required String roomCode}) async {
  ...
  Future<CreatedRoom> setReady({String? jwt, required String roomCode, required bool ready}) async {
